  ...
  template: `
    ...
    <ion-content>
      <div class="page-container">
        <div
          class="address-list"
          *ngIf="addressService.getCurrentAddresses().length > 0"
        >
          <div
            class="address-card"
            *ngFor="let address of addressService.getCurrentAddresses()"
          >
            ...
            <div class="address-body">
              ...
              <p class="address-text">
                {{ address.addressLine }}<br />{{ address.area }},
                {{ address.city }}
              </p>
              <p class="landmark" *ngIf="address.nearestLandmark">
                Landmark: {{ address.nearestLandmark }}
              </p>
            </div>
            <div class="actions">
              <button class="btn-action edit" (click)="edit(address)">
                Edit
              </button>
              <button
                class="btn-action delete"
                (click)="addressService.deleteAddress(address.id).subscribe()"
              >
                Delete
              </button>
              <button
                ...
                (click)="
                  addressService.setDefaultAddress(address.id).subscribe()
                "
          ...
          <div class="form-stack">
            <div class="map-placeholder">
              <div class="map-box">
                <ion-icon
                  name="map-outline"
                  style="font-size: 32px; color: #b05070; margin-bottom: 8px;"
                ></ion-icon>
  ...
  styles: [
    `
      ...
      .field:focus {
        ...
        box-shadow: 0 0 0 3px rgba(14, 168, 125, 0.1);
        background: var(--color-white-near-white);
      }
...
export class AddressesPage {
  ...
  save(): void {
    ...
    if (this.editingId) {
      this.addressService
        .updateAddress({ id: this.editingId, ...this.form })
        .subscribe(() => {
          if (redirectTo) {
